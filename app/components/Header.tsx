    ...
    <header className="relative md:fixed left-0 right-0 top-0 z-20 flex items-start justify-between px-3 py-2 pr-14 md:px-1 md:pr-1 lg:px-6 lg:pr-6 lg:py-1 bg-transparent">
      ...
      {mobileMenuOpen && (
        <div
          ...
          onClick={() => setMobileMenuOpen(false)}
        >
          <nav className="flex flex-col items-center justify-center gap-1 text-neutral-900 text-lg text-center [&_a]:text-neutral-900 [&_a:hover]:text-neutral-600 [&_a]:block [&_a]:text-center">
            {mobileNavLinks}
          </nav>
        </div>
      )}
